...
function parseArgs() {
  ...
  if (args.length === 0) {
    ...
    console.error('Usage: node convert_docx.js <file.docx> [--output-dir <dir>] [--archive-dir <dir>]');
    process.exit(1);
  }
...
/**
 * Validate that file exists and is a DOCX
 */
async function validateFile(filePath) {
  try {
    ...
    if (ext !== '.docx') {
      throw new Error(`${filePath} is not a DOCX file`);
    }
...
async function convertDocxToMarkdown(filePath) {
  try {
    ...
    const result = await mammoth.convertToMarkdown({ buffer: dataBuffer });
    
    let markdown = result.value;
    
    // Handle any messages/warnings
...
async function main() {
  try {
    ...
    const markdown = await convertDocxToMarkdown(absoluteFilePath);
    
    // Determine output path
    const fileDir = path.dirname(absoluteFilePath);
    const fileName = path.basename(absoluteFilePath, '.docx');
...
module.exports = { convertDocxToMarkdown, archiveFile };
